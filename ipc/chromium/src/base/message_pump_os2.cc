...
void MessagePumpOS2::AddObserver(Observer* observer) {
  observers_.AddObserver(observer);
}

void MessagePumpOS2::RemoveObserver(Observer* observer) {
  observers_.RemoveObserver(observer);
}

void MessagePumpOS2::WillProcessMessage(const QMSG& msg) {
  FOR_EACH_OBSERVER(Observer, observers_, WillProcessMessage(msg));
}

void MessagePumpOS2::DidProcessMessage(const QMSG& msg) {
  FOR_EACH_OBSERVER(Observer, observers_, DidProcessMessage(msg));
}

void MessagePumpOS2::RunWithDispatcher(
...
void MessagePumpForUI::PumpOutPendingPaintMessages() {
  ...
  for (peek_count = 0; peek_count < kMaxPeekCount; ++peek_count) {
    QMSG msg;
    if (!WinPeekMsg(0, &msg, NULLHANDLE, WM_PAINT, WM_PAINT, PM_REMOVE))
      break;
...
bool MessagePumpForUI::ProcessPumpReplacementMessage() {
  ...
  if (MessageLoop::current()->os_modal_loop()) {
    ...
    have_message = WinPeekMsg(0, &msg, NULLHANDLE, WM_PAINT, WM_PAINT, PM_REMOVE) ||
                   WinPeekMsg(0, &msg, NULLHANDLE, WM_TIMER, WM_TIMER, PM_REMOVE);
  } else {
    have_message = (0 != WinPeekMsg(0, &msg, NULLHANDLE, 0, 0, PM_REMOVE));

    if (have_message && msg.msg == WM_NULL)
      have_message = (0 != WinPeekMsg(0, &msg, NULLHANDLE, 0, 0, PM_REMOVE));
  }
  ...
  int old_have_work = __atomic_xchg(&have_work_, 0);
