...
#if defined(XP_OS2)
// On OS/2 PR_GetOSError returns a native OS/2 error but OS.File.Error expects
// errno (as we go Posix-way there). Provide a converter.
PRInt32 PR_GET_OS_ERROR()
{
  // We could use __libc_native2errno(PR_GetOSError()) under kLIBC but it is
  // not exported, unfortunately. So we have to copy-paste it from there. It is
  // important to get proper mappings, otherwise the "because" properties in
  // osfile_unix_allthreads.jsm won't work correctly causing variuos side effects.
  ...
  PRUint32 rc = PR_GetOSError();
  if (rc >= sizeof(errno_tab))
    return EINVAL;
  return errno_tab[rc];
}
#else
#define PR_GET_OS_ERROR() PR_GetOSError()
#endif
 ...
 private:
  ...
  nsresult Read(ScopedArrayBufferContents& aBuffer)
  {
    ...
    if (!file) {
      ...
      Fail(NS_LITERAL_CSTRING("ImportFile"), nullptr, PR_GET_OS_ERROR());
      return NS_ERROR_FAILURE;
    }
    ...
    if (!file) {
      Fail(NS_LITERAL_CSTRING("open"), nullptr, PR_GET_OS_ERROR());
      return NS_ERROR_FAILURE;
    }
    ...
    if (PR_GetOpenFileInfo64(file, &stat) != PR_SUCCESS) {
      Fail(NS_LITERAL_CSTRING("stat"), nullptr, PR_GET_OS_ERROR());
      return NS_ERROR_FAILURE;
    }
    ...
    do {
      ...
      if (just_read == -1) {
        Fail(NS_LITERAL_CSTRING("read"), nullptr, PR_GET_OS_ERROR());
        return NS_ERROR_FAILURE;
      }
