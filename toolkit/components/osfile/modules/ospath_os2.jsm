 ...
 *
 * This implementation is a slight modification of ospath_win.jsm and
 ...
 *
 * Under OS/2, this will return "$TMP\foo\bar".
 */
let join = function(...path) {
  ...
  for (let subpath of path) {
    ...
    if (drive) {
      ...
      absolute = true;
    } else if (this.os2IsAbsolute(subpath)) {
...
exports.os2GetDrive = os2GetDrive;
 ...
 * or "driveletter:\\".
 */
let os2IsAbsolute = function(path) {
  let index = path.indexOf(":");
  return path.length > index + 1 && (path[index + 1] == "\\" || path[index + 1] == "/");
};
exports.os2IsAbsolute = os2IsAbsolute;
...
let fromFileURI = function fromFileURI(uri) {
  ...
  // strip leading slash, since OS/2 paths don't start with one
