...
nsOSHelperAppService::GetFromExtension(const nsCString& aFileExt) {
  ...
  RefPtr<nsMIMEInfoOS2> mimeInfo = new nsMIMEInfoOS2(mimeType);
  if (!mimeInfo)
    return nullptr;
  NS_ADDREF(mimeInfo);
...
nsOSHelperAppService::GetFromType(const nsCString& aMIMEType) {
  ...
  RefPtr<nsMIMEInfoOS2> mimeInfo = new nsMIMEInfoOS2(aMIMEType);
  if (!mimeInfo)
    return nullptr;
  NS_ADDREF(mimeInfo);
...
nsOSHelperAppService::GetMIMEInfoFromOS(const nsACString& aType,
                                        ...
                                        bool       *aFound) {
  *aFound = true;
  RefPtr<nsMIMEInfoOS2> retval = GetFromType(PromiseFlatCString(aType));
  bool hasDefault = false;
  if (retval)
    retval->GetHasDefaultHandler(&hasDefault);
  if (!retval || !hasDefault) {
    RefPtr<nsMIMEInfoOS2> miByExt = GetFromExtension(PromiseFlatCString(aFileExt));
